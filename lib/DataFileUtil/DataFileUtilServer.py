...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'DataFileUtil'):
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'DataFileUtil'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "DataFileUtil but no authentication header was passed"
                            raise err
